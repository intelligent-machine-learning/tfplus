...
#ifndef TFPLUS_KV_VARIABLE_KERNELS_TENSOR_BUNDLE_H_
#define TFPLUS_KV_VARIABLE_KERNELS_TENSOR_BUNDLE_H_
#include <map>
#include <string>
...
class BundleWriter {
 public:
  ...
  Status AddSliceHeader(string tensor_name, const TensorShape& shape,
                        DataType type, bool is_hash, TensorSliceProto** proto);
  // tf plus unique
  Status AddStringTensor(StringPiece key, const std::vector<string*>& strings,
                         TensorShape shape);

  Status BeginWriteChunkData(StringPiece key,
                             DataType dtype,
                             TensorShape shape);
  // The bytes to be writed is less than file buffer size
  Status WriteOneBufferData(char* content, int64 data_bytes_written);
  // Many chunk, needs to be splits
  Status WriteChunkData(char* content, int64 data_bytes_written);
  void EndWriteChunkData(int64 total_bytes_written, int64 end_bytes_written);
...
class BundleReader {
 public:
  ...
  // REQUIRES: status().ok()
  int CalcNumShardsByTensorName(const string& prefix_name,
                                const string& suffix_name);
  bool PartExists(int part_id, const string& prefix_name,
                  const string& suffix_name);
  ...
  Status LookupSegment(StringPiece key, size_t buffer_size, char* destination,
                       size_t& real_bytes_read); // NOLINT
  Status LookupSegmentOffset(StringPiece key, uint64_t offset,
                             size_t buffer_size, char* destination,
                             size_t& real_bytes_read); // NOLINT

  Status GetTensorInfo(StringPiece key, int64* size,
                       std::unique_ptr<RandomAccessFile>* file, int64* offset);
  // Looks up the tensor keyed by "key" and filtered by indices. If "key" refers
  // to a partitioned tensor, attempts to look up the full contents using all
  // stored slices.
  //
  // Caller must make sure "val" has the same shape and dtype as the
  // corresponding contents, so that its buffer can be filled without needing
  ...
  // Validates the stored crc32c checksum against the restored bytes.
  // REQUIRES: status().ok()
  Status LookupWithIndices(StringPiece key, Tensor* val,
  ...
  Status LookupTensorSliceProtos(StringPiece key,
 ...
 private:
  ...
  // Reads the tensor value described by the metadata proto "entry".
  // Filters the tensor value with the specified indices.
  // Usage for "val" follows the comment of "Lookup()".
  Status GetValueWithIndices(const BundleEntryProto& entry, Tensor* val,
                             const std::vector<int64>& indices)
      TF_MUST_USE_RESULT;
...
class FileOutputBuffer {
 public:
  ...
  Status AppendChunk(StringPiece data);
...
class SegmentBundleWriter {
 public:
  SegmentBundleWriter(BundleWriter* writer, const string& name,
                      const TensorShape& shape, DataType type,
                      int64 buffer_size = 4 << 20);
  Status Begin();
...
class SegmentBundleReader {
 public:
  SegmentBundleReader(BundleReader* reader, const string& name, int64 offset,
                      int64 size, int64 buffer_size = 4 << 20);
  Status Begin();
...
#endif  // TFPLUS_KV_VARIABLE_KERNELS_TENSOR_BUNDLE_H_
