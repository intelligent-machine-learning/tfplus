...
class OSSConnection {
 public:
  OSSConnection(const std::string& endPoint, const std::string& accessKey,
    ...
    _options->ctl->options->connect_timeout =
        GetEnvOrDefault<int>("OSS_CONNECT_TIMEOUT", 60);
    _options->ctl->options->dns_cache_timeout =
        GetEnvOrDefault<int>("OSS_DNS_CACHE_TIMEOUT", 5 * 60);
  }
 ...
 public:
  ...
  Status Read(uint64 offset, size_t n, StringPiece* result,
    ...
    if (offset + n >= total_file_length_) {
      remaining = static_cast<int64>(total_file_length_) - offset;
    }
...
class OSSWritableFile : public WritableFile {
 public:
  ...
  Status Append(StringPiece data) override {
    mutex_lock lock(mu_);
    TF_RETURN_IF_ERROR(CheckClosed());
    InitAprPool();
    if (CurrentBufferLength() >= part_size_) {
      TF_RETURN_IF_ERROR(FlushInternal());
    }
...
Status OSSFileSystem::ParseOSSURIPath(const StringPiece fname,
                                      std::string* bucket, std::string* object,
                                      std::string* host, std::string* access_id,
  ...
  *bucket = string(bucketp.substr(0, pos));
  ...
  for (const auto& key_value : access_infos) {
    ...
    if (pos == StringPiece::npos) {
      return errors::InvalidArgument("OSS path access info failed: ", fname,
                                     " info:", key_value);
    }
...
Status OSSFileSystem::NewRandomAccessFile(
  ...
  TF_RETURN_IF_ERROR(ParseOSSURIPath(filename, &bucket, &object, &host,
                                     &access_id, &access_key));
  FileStatistics stat;
  OSSConnection conn(host, access_id, access_key);
  TF_RETURN_IF_ERROR(RetrieveObjectMetadata(
...
Status OSSFileSystem::NewWritableFile(const std::string& fname,
  ...
  std::string host, access_id, access_key;
  TF_RETURN_IF_ERROR(
      ParseOSSURIPath(fname, &bucket, &object, &host, &access_id, &access_key));
...
Status OSSFileSystem::ListObjects(
...
Status OSSFileSystem::StatInternal(aos_pool_t* pool,
                                   const oss_request_options_t* options,
                                   const std::string& bucket,
                                   const std::string& object,
                                   FileStatistics* stat) {
  Status s = RetrieveObjectMetadata(pool, options, bucket, object, stat);
  if (s.ok()) {
  ...
  s = RetrieveObjectMetadata(pool, options, bucket, objectName, stat);
  if (s.ok()) {
  ...
  s = ListObjects(pool, options, bucket, object, &listing, true, false, false,
                  10);
...
Status OSSFileSystem::RetrieveObjectMetadata(
    aos_pool_t* pool, const oss_request_options_t* options,
    const std::string& bucket, const std::string& object,
    FileStatistics* stat) {
  ...
  if (aos_status_is_ok(status)) {
    content_length_str =
        const_cast<char*>(apr_table_get(resp_headers, OSS_CONTENT_LENGTH));
    ...
    object_date_str = const_cast<char*>(apr_table_get(resp_headers, OSS_DATE));
...
Status OSSFileSystem::Stat(const std::string& fname, FileStatistics* stat) {
  ...
  return StatInternal(pool, ossOptions, bucket, object, stat);
}
...
Status OSSFileSystem::GetChildren(const std::string& dir,
  ...
  std::string host, access_id, access_key;
  TF_RETURN_IF_ERROR(
      ParseOSSURIPath(dir, &bucket, &object, &host, &access_id, &access_key));
  OSSConnection oss(host, access_id, access_key);
  oss_request_options_t* oss_options = oss.getRequestOptions();
  aos_pool_t* pool = oss.getPool();
  return ListObjects(pool, oss_options, bucket, object, result, true, false,
                     true, 1000);
}
...
Status OSSFileSystem::DeleteObjectInternal(const oss_request_options_t* options,
                                           const std::string& bucket,
                                           const std::string& object) {
  aos_string_t bucket_;
  aos_string_t object_;
...
Status OSSFileSystem::DeleteFile(const std::string& fname) {
  ...
  return DeleteObjectInternal(oss_options, bucket, object);
}
...
Status OSSFileSystem::CreateDir(const std::string& dirname) {
  ...
  std::string object, bucket;
  std::string host, access_id, access_key;
  TF_RETURN_IF_ERROR(ParseOSSURIPath(dirname, &bucket, &object, &host,
                                     &access_id, &access_key));
  OSSConnection oss(host, access_id, access_key);
  oss_request_options_t* ossOptions = oss.getRequestOptions();
  aos_pool_t* pool = oss.getPool();
  ...
  if (splitPaths.size() < 2) {
    return CreateDirInternal(pool, ossOptions, bucket, object);
  }

  FileStatistics stat;
  StringPiece parent = io::Dirname(dirs);

  if (!StatInternal(pool, ossOptions, bucket, string(parent), &stat).ok()) {
  ...
  TF_RETURN_IF_ERROR(CreateDirInternal(pool, ossOptions, bucket, object));
  return Status::OK();
}
...
Status OSSFileSystem::RecursivelyCreateDir(const string& dirname) {
  ...
  for (auto path : splitPaths) {
    ...
    if (!CreateDirInternal(pool, ossOptions, bucket, dir).ok()) {
...
Status OSSFileSystem::CreateDirInternal(aos_pool_t* pool,
                                        const oss_request_options_t* options,
                                        const std::string& bucket,
                                        const std::string& dirname) {
  FileStatistics stat;
  if (RetrieveObjectMetadata(pool, options, bucket, dirname, &stat).ok()) {
    if (!stat.is_directory) {
...
Status OSSFileSystem::DeleteDir(const std::string& dirname) {
  ...
  s = ListObjects(pool, oss_options, bucket, object, &children, true, false,
                  false, 10);
  if (s.ok() && !children.empty()) {
    return errors::FailedPrecondition("Cannot delete a non-empty directory.");
  }

  s = DeleteObjectInternal(oss_options, bucket, object);
  ...
  return DeleteObjectInternal(oss_options, bucket, object.append(kDelim));
}
...
Status OSSFileSystem::RenameFile(const std::string& src,
  ...
  std::string host, access_id, access_key;
  TF_RETURN_IF_ERROR(
      ParseOSSURIPath(src, &sbucket, &sobject, &host, &access_id, &access_key));
  std::string dobject, dbucket;
  std::string dhost, daccess_id, daccess_key;
  TF_RETURN_IF_ERROR(ParseOSSURIPath(target, &dbucket, &dobject, &dhost,
  ...
  if (status.ok()) {
    ...
    for (const auto& child : childPaths) {
      ...
      DeleteObjectInternal(oss_options, sbucket, tmp_sobject);
    }
  ...
  return DeleteObjectInternal(oss_options, sbucket, sobject);
}

aos_status_t* OSSFileSystem::RenameFileInternal(
  ...
  if (file_size > upload_part_bytes_) {
    ...
    int parts = ceil(static_cast<double>(file_size) /
                     static_cast<double>(upload_part_bytes_));
...
Status OSSFileSystem::DeleteRecursively(const std::string& dirname,
  ...
  *undeleted_dirs = 0;
  ...
  FileStatistics stat;
  Status s;
  s = StatInternal(pool, oss_options, bucket, object, &stat);
  if (!s.ok() || !stat.is_directory) {
    *undeleted_dirs = 1;
    return errors::NotFound(dirname, " doesn't exist or not a directory.");
  }

  s = ListObjects(pool, oss_options, bucket, object, &children, true, true,
                  false, 1000);
  if (!s.ok()) {
  ...
  for (const auto& child : children) {
    s = DeleteObjectInternal(oss_options, bucket, child);
    if (!s.ok()) {
      s = StatInternal(pool, oss_options, bucket, child, &stat);
      if (s.ok()) {
        if (stat.is_directory) {
          ++*undeleted_dirs;
        } else {
